;--------------------------------------------------------------------
; Practice 20: Actual Code On Progress Four on PIC18F4620
...
; 9. Microswitch Interrupt Service Routine at Port B7-B4, Change Keypad to Pins E..., it gets activated when pin b changes state  
	; B0, B1 -> Microswitch input with interrupts(according to yihtang, double check!)
...
; 4. Changing Ports to actual ports 
	...
	; C7-C4 => Solenoids             DONE           (RED) 
		; C7 = 
		; C6 = 
		; C5 = 
		; C4 = 
	; C3, C0 => both motors 		DONE			(BLUE) 
...
; CHANGE THIS INTCON LINE AS YOU"RE USING INTERRUPTS!!!!!, build interrupt debouncing circuit. 
...
; Set Inputs/Outputs (TRISX) 
         ...
         movlw	b'11111110'	; Set required keypad inputs
						...
						;DEBUG
						;				clrf TRISB 
         movwf	TRISB ; 11111110 -> B7-B4 Keypad Inputs, B3 => Keypad Enable 
		 clrf	TRISC       ; 00000000 -> C7-C4 Solenoids, C3,C0 => Both Motor, C2,C1 => Bidirectional motor 
		 clrf 	TRISD		; 00000000 -> D7-D4 LCD Data, D3,D2 E & RS for LCD 
		 movlw	b'011'		; Set required microswitch inputs
...
;			movf		tempwsaveregister, W
		...
		sublw		0x07  ; subtract w with 000111 and place result in w
		...
		sublw		0x04 ; The math used is 2... method, where it loops back in this case 3 - 7 - 4 = 0. 
...
Dispenser: ; Will come here 1st time right after above code, and right after 6 dowels are loaded onto box (Flipper) 
		...
		call Stepper_Motor_OnceHalf ; to move the 2nd box into place, need to move stepper motor twice. 		
;--------------------------------------------------------------------
; Box Positioner						
;--------------------------------------------------------------------
BoxPositioner:
...
Stepper_Motor_2 ; Counter-clockwise
		bsf     LATA, 1
		call DelayStepperMotor ; Need delay to move motor
		bcf     LATA, 1
		bsf     LATA, 2
...
; should be 33.. but need 40 since first few stop sit. 
...
Stepper_Motor_OnceHalf
	call Stepper_Motor_26
	return 
